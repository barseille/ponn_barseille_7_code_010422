<template>
  <v-app id="moderation" class="moderation">
    ...
    <div class="ml-12">
      ...
      <v-container>
        <v-btn class="ma-3" color="black white--text" @click="clickPosts">
          Voir les posts
        </v-btn>
        <v-btn class="ma-3" color="black white--text" @click="clickComments">
        ...
        <div v-if="showComments">
          <v-card
            class="forum__comments--ind my-1 mx-2 pa-0"
            color="#ECECEC"
          ...
          >
            ...
            <v-card-actions class="forum__comments__manage">
              <v-btn
                ...
                class="forum__comments__manage--btn"
...
export default {
  ...
  methods: {
    ...
    deletePost(pId) {
      axios
        .delete("http://localhost:3000/api/moderation/post/" + pId, {
    ...
    deleteComment(cId) {
      axios
        .delete("http://localhost:3000/api/moderation/comment/" + cId, {
          headers: { Authorization: "Bearer " + localStorage.token },
        })
        .then((response) => {
          let rep = JSON.parse(response.data);
  ...
  mounted() {
    axios
      .get("http://localhost:3000/api/moderation/posts", {
        headers: { Authorization: "Bearer " + localStorage.token },
      })
      .then((response) => {
    ...
    axios
      .get("http://localhost:3000/api/moderation/comments", {
        headers: { Authorization: "Bearer " + localStorage.token },
      })
      .then((response) => {
