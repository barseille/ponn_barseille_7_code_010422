  ...
  <div id="profil" class="d-flex justify-center">
    ...
    <v-card class="ma-12" raised>
      ...
      <v-card-actions class="d-flex justify-space-between">
        <v-btn @click.stop="dialogUp = true" title="modifier mes informations">
          Modifier
        </v-btn>
    ...
    <v-dialog v-model="dialogDel" max-width="350px">
      <v-card>
        ...
        <v-card-text>
          <p>
            En supprimant votre profil, vous effacerez aussi tous vos posts
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn text @click="dialogDel = false"> Annuler </v-btn>

          <v-btn text @click="deleteUser"> Supprimer mon profil </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
export default {
  ...
  methods: {
    ...
    updateUser() {
      ...
      axios
        .put("http://localhost:3000/api/auth/", this.dataUpS, {
          headers: {
